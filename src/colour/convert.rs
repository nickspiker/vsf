...
//! `Source → linear → VSF RGB → linear → Target`; there is no XYZ or LMS hub.
//! Each supported space has one matrix into VSF RGB and one out, generated by
//! `tools/generate_constants.rs`. How that matrix is derived depends on how the
//! space defines its primaries (full policy and current status: `colour.md`,
//! "Converting Between Colourspaces"):
//!
//! - **Wavelength-defined** (VSF RGB, Rec.2020): primaries evaluated under the
//!   Stockman & Sharpe 2000 10° cone fundamentals, matrix built in LMS.
//! - **xy-defined** (sRGB/Rec.709, Adobe RGB, ...): an xy pair is a 1931
//!   chromaticity and nothing else, so the matrix is built xy → 1931 XYZ with
//!   VSF's primaries placed under the same 1931 CMFs. 1931 is confined to the
//!   entry matrix of the legacy space.
//! - **Chromatic adaptation**: legacy whites are D65, VSF's is E; adaptation
//!   is a von Kries scaling in SS2000 10° LMS folded into the entry matrix.
//!
//! ## Why CIE 1931 is `legacy`
//!
//! The 1931 observer comes from 1920s matching experiments on ~17 subjects and
//! has known flaws, worst in the blue. A colourspace whose primaries are xy
//! coordinates is bound to that observer forever: there is no spectrum behind
//! the numbers to re-evaluate. VSF RGB's primaries are wavelengths, so its
//! matrices are recomputed whenever a better observer is adopted and the
//! colourspace itself never changes.
//!
...
//!   - xy-defined, so 1931 XYZ entry matrix
//!
//! - **Rec.2020 / BT.2020**: Uses their WAVELENGTH specification (630nm, 532nm, 467nm)
//!   - Their xy coordinates were derived from those wavelengths under 1931; the wavelengths are the physical stimulus, so we use those
...
impl VsfType {
    ...
    ///
    /// Based on the Stockman & Sharpe 2000 10° cone fundamentals.
    ...
    ///
    /// LMS represents Long, Medium, and Short wavelength cone responses. Based on the Stockman & Sharpe 2000 10° cone fundamentals.
    ///
    /// # Arguments
...
mod tests {
    ...
    fn test_bt2020_conversion() {
        // Test BT.2020 white (studio range [16,235]) → VSF RGB. Float storage so this measures the matrix, not the ×256 u8 clamp (255/256 squared is 0.992 at full scale).
        let bt2020_white = VsfType::from_rec2020_f32(235u8, 235u8, 235u8, ColourFormat::Rf);
        let vsf_white = bt2020_white.to_rgb_linear_f32().unwrap();

        // The Rec.2020 entry matrix carries a von Kries D65 → E adaptation, so Rec.2020 white is a VSF neutral: R = G = B = 1 after studio-range expansion.
        for (name, v) in [("r", vsf_white.r), ("g", vsf_white.g), ("b", vsf_white.b)] {
            assert!((v - 1.0).abs() < 1e-4, "{name}={v}");
        }
